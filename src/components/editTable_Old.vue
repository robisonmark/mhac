      ...
      <slot name="thead">
        <tr>
          <template v-for="(column, key, index) in columns">
            <th v-if="!column.field_name.includes('id')" :key="index" :class="column.name.toLowerCase()">{{ column.name }}
            </th>
          </template>
          <th></th>
        </tr>
        ...
        <tr class="split-fields" v-else>
          ...
          <template v-for="(field, index) in columns" key="index">
            <td v-if="!field.field_name.includes('id') && !field.field_name.includes('actions')" class="input-con">
              ...
              <div v-else-if="field.type === 'customSelect'" tabindex="0" @click="changeDisplay(field.field_name)"
                @keyup.space="changeDisplay(field.field_name)" :class="{ 'vs': value[field.field_name] }"
                class="currentCustom">
              ...
              <multiselect v-else-if="field.type === 'multiselect' && field.field_name === 'season_roster'"
                ...
                :options="selectOptions(field.field_name)" :closeOnSelect=false :multiple="true" :taggable="true"
                @tag="addTag"></multiselect>

              <multiselect v-else-if="field.type === 'multiselect' && field.field_name === 'level'"
                v-model="value[field.model]" label="level_name" :options="selectOptions(field.field_name)" track-by="id"
                :closeOnSelect="false" :multiple="true" :taggable="true" @tag="addTag"></multiselect>
